...
export class N8nAdapter implements MCPBackendAdapter {
      ...
      switch (name) {
        case 'list_workflows':
          ...
          if (args.active !== undefined) {
            workflows = workflows.filter((w: any) => w.active === args.active);
          }
          
          if (args.tags?.length) {
            workflows = workflows.filter((w: any) => 
              args.tags.some((tag: string) => w.tags?.includes(tag))
            );
          }
        ...
        case 'execute_workflow':
          const executeResponse = await this.client.post(
            `/api/v1/workflows/${args.workflowId}/execute`,
            { data: args.data || {} }
          );
        ...
        case 'get_workflow':
          const workflowResponse = await this.client.get(
            `/api/v1/workflows/${args.workflowId}`
          );
          return workflowResponse.data;

        case 'activate_workflow':
          const activateResponse = await this.client.patch(
            `/api/v1/workflows/${args.workflowId}`,
            { active: args.active }
          );
        ...
        case 'get_execution':
          const executionResponse = await this.client.get(
            `/api/v1/executions/${args.executionId}`,
            { params: { includeData: args.includeData || false } }
          );
