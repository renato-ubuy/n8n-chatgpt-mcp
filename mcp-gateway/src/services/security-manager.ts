...
export class SecurityManager extends EventEmitter {
  ...
  async validateRequest(request: JsonRpcRequest, token: GatewayToken): Promise<ValidationResult> {
    try {
  ...
  private validateRequestParameters(request: JsonRpcRequest): ValidationResult {
    try {
  ...
  private checkSuspiciousPatterns(request: JsonRpcRequest): ValidationResult {
  ...
  private validateRateLimit(token: GatewayToken): ValidationResult {
  ...
  async scanBackend(backend: MCPBackend): Promise<void> {
    try {
  ...
  async logIncident(incident: Omit<SecurityIncident, 'id'>): Promise<void> {
  ...
  getSecurityStatus(): SecurityStatus {
    ...
    const recentIncidents = this.incidents.filter(
      incident => (now - incident.timestamp.getTime()) < (24 * 60 * 60 * 1000) // Last 24 hours
    );
    ...
    let securityLevel: SecurityStatus['securityLevel'] = 'secure';
